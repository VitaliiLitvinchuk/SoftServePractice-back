...
{
    public async Task<Result<MovieGenre, MovieGenreException>> Handle(CreateMovieGenreCommand request, CancellationToken cancellationToken)
    {
        ...
        return await result.Match(
            ...
            async () =>
            {
                var movie = await movieQuery.Get(cancellationToken, x => x.Id == entity.MovieId);

                return await movie.Match(
                    async movie =>
                    {
                        var genre = await genreQuery.Get(cancellationToken, x => x.Id == entity.GenreId);

                        return await genre.Match(
                            async genre => await CreateEntity(entity, cancellationToken),
